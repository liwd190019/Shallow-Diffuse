...
def store_pil_image(image, output_file_path):
...
def store_imgs(img_class, output_folder_path):
    # Ensure the output directory exists
    os.makedirs(output_folder_path, exist_ok=True)

    for attr_name, attr_value in vars(img_class).items():
        if attr_name == 'None': continue # skip the non-attacked images
        if isinstance(attr_value, Image.Image):  # Check if it's an image object
            img_filename = f"{attr_name}.png"
            img_path = os.path.join(output_folder_path, img_filename)
            attr_value.save(img_path)
